...
PostProcessPixelOutput main(PostProcessVertexToPixel input)
{
    ...
    float3 resource = FullscreenTexture.Sample(DefaultSampler, input.myUV.xy).rgb;
    
    // Levels
	{
        float luminance = dot(float3(0.2126, 0.7152, 0.0722), resource); // convert to grayscale
        resource = luminance + mySaturation * (resource - luminance);
        resource = exp2(myExposure) * resource;
        resource = 0.18f * pow(resource / 0.18f, myContrast); // 0.18 is perceived as 50% grey. Keep it constant
        resource *= myTint;
        resource = max(0.0f, resource - myBlackpoint);
    }
