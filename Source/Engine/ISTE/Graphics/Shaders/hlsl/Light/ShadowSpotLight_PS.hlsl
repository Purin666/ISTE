...
float4 main(float4 aPosition : SV_Position) : SV_TARGET
{
    ...
    float2 texOffset = float2(ddx(uv.x), ddy(uv.y));
    
    
    
    
    float shadowFactor = 1.f;
    float4 ProjectedPositionTemp = mul(myLightProjection, worldPos);
        ...
        clamp(ProjectedPosition.y, -1.0, 1.0) == ProjectedPosition.y)
    {
        for (float i = -n; i < n; i++)
            for (float j = -n; j < n; j++)
            {
                ...
                uvPos += offset;
                float shadowMapZ = ShadowMap.Sample(ShadowSampler, uvPos);
            
                shadowFactor += (computedZ < (shadowMapZ + bias));
            }
        shadowFactor /= pow(n * 2, 2);
    }
    return float4(spotLightColor * shadowFactor, 1);
